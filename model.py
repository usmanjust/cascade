...
class GCLRec(nn.Module):
    ...
    def computer(self, graph_type='original'):
        """
        Propagation for LightGCN — modified to support AMP (float16/bfloat16) safely.
        Runs sparse.mm in float32 to avoid CUDA half precision crash.
        """
        # Combine basket and item embeddings
        ...
        # Ensure sparse matrix is ready (float32)
        g = g.coalesce().to(self.config['device']).float()

        orig_dtype = all_emb.dtype
        ...
        light_out = light_out.to(orig_dtype)

        # Split back into basket/item embeddings
